...
export default function Otp({helper, otp, setOtp, status}) {
...
console.log(status)
    return(
        <div style={{backgroundColor: 'white', display: 'flex', flexDirection: 'row', borderRadius: 5, justifyContent: 'center'}}>
            ...
            maxLength="1"
            value={otp.slice(0,1)}
            style={{fontSize: 24, width: 38, height: 40, display: 'flex',boxShadow: '0 0 5pt 0.5pt #D3D3D3', 
            alignItems: 'center',borderRadius: 8,borderColor:`${status}`, justifySelf: 'center', textAlign: 'center', marginRight: 5}} 
            onChange={(event) => {
            ...
            maxLength="1"
            value={otp.slice(1,2)}
            style={{fontSize: 24, width: 38, height: 40, borderColor:`${status}`,display: 'flex',boxShadow: '0 0 5pt 0.5pt #D3D3D3', alignItems: 'center', borderRadius: 8,justifySelf: 'center', textAlign: 'center', marginRight: 5}} 
            onChange={(event) => {
                if(event.target.value !== '' && event.target.value.length===1 && isNumeric(event.target.value)) {
                    setOtp(otp + event.target.value);
                    ref2.current.focus();
            ...
            value={otp.slice(5)}
            style={{fontSize: 24, width: 38, height: 40,borderColor:`${status}`, display: 'flex',boxShadow: '0 0 5pt 0.5pt #D3D3D3', alignItems: 'center',borderRadius: 8, justifySelf: 'center', textAlign: 'center'}}
            onChange={(event) => {
                if(event.target.value !== '' && event.target.value.length===1 && isNumeric(event.target.value)) {
                    setOtp(otp + event.target.value);
                    ref5.current.blur();
